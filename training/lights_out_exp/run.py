...
from training.constants import (
    ...
    LIGHTS_OUT_EXP_TOKENIZER_DIR, LIGHTS_OUT_EXP_DATA_DIR,
)
...
DATA_CONFIGS = {
    "incomplete": {
        "jsonl_path": f"{LIGHTS_OUT_EXP_DATA_DIR}/tokenized.jsonl",
        "splits_path": f"{LIGHTS_OUT_EXP_DATA_DIR}/tokenized.splits.json",
    },
}
...
def make_experiment(exp: dict) -> tuple[str, dict]:
    ...
    name += f"_gn{e['max_grad_norm']}"
    name += f"_s{e['seed']}"
    name += f"_ms{e['max_steps']}"
    name += "_csa"
    ...
    config = {
        ...
        "paths": {
            ...
            "tokenizer_dir": LIGHTS_OUT_EXP_TOKENIZER_DIR,
            "output_dir": f"scratch_data/training_outputs/lights_out_exp_anonym_loc/{name}",
        ...
        "data": {
            ...
            "eval_num_workers": 10,
            "allow_cross_sample_attention": True,
        ...
        "wandb": {
            "project": "plan-verification-lights-out-exp",
            "run_name": name,
        },
    }
    return name, config
